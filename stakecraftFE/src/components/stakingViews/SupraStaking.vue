<template>
  <div class="supra-staking">
    <h3>Supra Staking</h3>
    
    <div v-if="!isConnected" class="connect-wallet">
      <button @click="connectStarkey" class="connect-btn">Connect Starkey Wallet</button>
    </div>

    <div v-else class="staking-interface">
      <div class="stake-info">
        <div class="balance">
          <span>Your SUPRA Balance:</span>
          <span>{{ formatBalance(supraBalance) }} SUPRA</span>
        </div>
        <div class="staked">
          <span>Currently Staked:</span>
          <span>{{ formatBalance(stakedAmount) }} SUPRA</span>
        </div>
        <div class="rewards">
          <span>Available Rewards:</span>
          <span>{{ formatBalance(rewards) }} SUPRA</span>
        </div>
      </div>

  ...
  for (const endpoint of RPC_ENDPOINTS) {
    try {
      const client = await SigningStargateClient.connectWithSigner(
        endpoint,
        offlineSigner,
        { gasPrice: GasPrice.fromString('0.0025ubtsg') } // Bitsong's native token
      )
      return client
    } catch (error) {
  ...
  try {
    ...
    const msg = {
      ...
      value: {
        ...
        amount: {
          denom: 'ubtsg',
          amount: String(amount * 1_000_000) // Convert to micro-BTSG
        }
      }
    }

    const result = await client.signAndBroadcast(delegatorAddress, [msg], {
      amount: [{ denom: 'ubtsg', amount: String(amount * 1_000_000) }],
      gas: '200000'
    })

...
const STAFI_ENDPOINTS = ['wss://stafi-rpc.stafi.io', 'wss://stafi-rpc.dwellir.com']

let api = null
let currentAccount = null

// Connect to Stafi network
const connectToStafi = async () => {
  if (api) return api

  let lastError = null
  for (const endpoint of STAFI_ENDPOINTS) {
    try {
      const provider = new WsProvider(endpoint)
      api = await ApiPromise.create({ provider })
      await api.isReady
      console.log(`Connected to Stafi via ${endpoint}`)
      return api
    } catch (error) {
  ...
  throw new Error(`Failed to connect to any Stafi endpoint. Last error: ${lastError?.message}`)
}
...
export const connectWallet = async () => {
  try {
    ...
    const extensions = await web3Enable('Stafi Staking')
...
export const getBalance = async (address) => {
  try {
    const api = await connectToStafi()
    const { data: balance } = await api.query.system.account(address)

    // Convert from smallest unit to FIS (assuming 12 decimals for Stafi)
    return parseFloat(balance.free.toString()) / Math.pow(10, 12)
  } catch (error) {
...
export const getStakingInfo = async (address, validatorAddress) => {
  try {
    ...
    if (stakingLedger.isSome) {
      ...
      stakedAmount = parseFloat(ledger.active.toString()) / Math.pow(10, 12)
    }
    ...
    try {
      ...
      if (pendingRewards) {
        stakingRewards = parseFloat(pendingRewards.toString()) / Math.pow(10, 12)
      }
    } catch (rewardError) {
...
export const stakeTokens = async (address, validatorAddress, amount) => {
  try {
    ...
    // Convert amount to smallest unit (assuming 12 decimals for Stafi)
    const stakeAmount = (amount * Math.pow(10, 12)).toString()
...
export const unstakeTokens = async (address, validatorAddress, amount) => {
  try {
    ...
    // Convert amount to smallest unit (assuming 12 decimals for Stafi)
    const unstakeAmount = (amount * Math.pow(10, 12)).toString()
...
export const getUnbondingInfo = async (address) => {
  try {
    ...
    return unbondingChunks.map((chunk) => ({
      amount: parseFloat(chunk.value.toString()) / Math.pow(10, 12),

...
const BAND_CHAIN_ID = 'laozi-mainnet' // Band Protocol mainnet chain ID

const RPC_ENDPOINTS = [
  'https://rpc-bandchain-ia.cosmosia.notional.ventures',
  'https://rpc-band.publicnode.com',
  'https://rpc-bandchain-ia.cosmosia.notional.ventures'
]
  ...
  for (const endpoint of RPC_ENDPOINTS) {
    try {
      const client = await SigningStargateClient.connectWithSigner(
        endpoint,
        offlineSigner,
        { gasPrice: GasPrice.fromString('0.0025uband') } // Band's native token
      )
      return client
    } catch (error) {
  ...
  try {
    ...
    const msg = {
      ...
      value: {
        ...
        amount: {
          denom: 'uband',
          amount: String(amount * 1_000_000) // Convert to micro-BAND
        }
      }
    }

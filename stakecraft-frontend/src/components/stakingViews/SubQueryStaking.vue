        ...
        <div class="modal-content">
          ...
          <div v-if="walletError" class="wallet-warning">
            ...
            <div class="warning-content">
              ...
              <p class="warning-message">
                To use SubQuery staking, you need to install the MetaMask wallet extension.
              </p>
            </div>
          </div>
          ...
          <div v-if="walletConnected">
            ...
            <div v-if="activeTab === 'stake'" class="tab-content">
              <div class="staking-form">
                <div class="form-group">
                  <label class="form-label">Amount to Delegate (SQT)</label>
                  <div class="input-container">
                    ...
                    />
                    <div class="input-suffix"><span>SQT</span></div>
                  </div>
                  <div class="input-hint">
                    <span>Minimum: {{ minimumStake }} SQT</span>
                    <button
                      @click="stakeAmount = Number(sqtBalance)"
                      class="max-button"
                      :disabled="Number(sqtBalance) <= 0"
                    >
                      Max
                    </button>
                  </div>
                ...
                <div v-if="!hasApproval && stakeAmount > 0" class="approval-section">
                  <div class="approval-info">
                    <p>First, approve SQT tokens for delegation:</p>
                    <button
                      @click="handleApproval"
                      :disabled="isProcessing"
                      class="primary-button full-width approval-button"
                    >
                      {{ isProcessing ? 'Approving...' : `Approve ${stakeAmount} SQT` }}
                    </button>
                  </div>
                </div>
                ...
                <div class="info-card">
                  ...
                  <div class="info-card-content">
                    <div class="info-row">
                      ...
                      <span class="info-value">{{ sqtBalance }} SQT</span>
                    </div>
                    <div class="info-row">
                      <span class="info-label">Currently Delegated:</span>
                      <span class="info-value">{{ stakedAmount }} SQT</span>
                    </div>
                    <div class="info-row">
                      <span class="info-label">Delegation Rewards:</span>
                      <span class="info-value">{{ stakingRewards }} SQT</span>
                    </div>
                  </div>
                </div>

                <div v-if="stakingSuccess" class="success-message">
                  Successfully delegated {{ stakeAmount }} SQT!
                </div>
                ...
                >
                  {{ isProcessing ? 'Processing...' : 'Delegate SQT Tokens' }}
                </button>
              </div>
            </div>
            ...
            <div v-if="activeTab === 'unstake'" class="tab-content">
              <div class="staking-form">
                <div class="form-group">
                  <label class="form-label">Amount to Undelegate (SQT)</label>
                  <div class="input-container">
                    <input
                  ...
                  <div class="input-hint">
                    <span>Available to undelegate: {{ stakedAmount }} SQT</span>
                    <button
                ...
                <div class="warning-card">
                  ...
                  <div class="warning-text">
                    <strong>Important:</strong> Undelegated SQT tokens will be locked for 28 days
                    before becoming available for withdrawal.
                  </div>
                </div>

                <div v-if="unstakingSuccess" class="success-message">
                  Successfully undelegated {{ unstakeAmount }} SQT!
                </div>
                ...
                >
                  {{ isProcessing ? 'Processing...' : 'Undelegate SQT Tokens' }}
                </button>
              </div>
            </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const isValidStake = computed(() => {
      ...
      return !isNaN(amount) && amount >= minimumStake && amount <= Number(sqtBalance.value)
    })
    ...
    const handleDisconnectWallet = () => {
      try {
        ...
        stakingRewards.value = 0 // Assuming rewardsEarned and lastRewardTime are stakingRewards
        transactionHash.value = ''
...
<style scoped>
/* Same styles as previous MetaMask components */
.modal-fade-enter,
.modal-fade-leave-to {
  opacity: 0;
}
